...
def test_run_load_ok(monkeypatch):
    ...
    monkeypatch.setattr(eng, "httpx", types.SimpleNamespace(AsyncClient=DummyAsyncClient))

    lat, codes, errors = asyncio.run(
        run_load(
            url="https://service.ok",
            rps=5, duration=1, method="GET",
            timeout=5.0, headers={}, body=None, verify_ssl=True
        )
    )
    assert len(codes) == 5
