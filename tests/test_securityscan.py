...
def test_fetch_headers_http1_fallback():
    ...
    respx.get(f"{TEST_URL}/").mock(return_value=httpx.Response(200, headers={"Server": "Apache"}))
...
def test_tls_info_and_analyze_tls(monkeypatch):
    class FakeSSock:
        def getpeercert(self):
            exp = (datetime.datetime.utcnow() + datetime.timedelta(days=45)).strftime("%b %d %H:%M:%S %Y GMT")
            return {"notAfter": exp}
        def cipher(self):
...
def test_cli_run_writes_json(tmp_path, monkeypatch):
    ...
    fake_info = {"version": "TLSv1.3", "cipher": ("TLS_AES_256_GCM_SHA384", "", 256), "expires_at": None}
