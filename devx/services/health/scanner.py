...
from devx.core.utils import bytes_to_mb, iter_files
...
def outdated(requirements: Path):
    if not requirements.exists(): return []
    try:
        out = subprocess.check_output([sys.executable, "-m", "pip", "list", "--outdated", "--format=freeze"], text=True)
        outdated = {line.split("==")[0].lower() for line in out.splitlines() if "==" in line}
        req = {line.split("==")[0].strip().lower() for line in requirements.read_text().splitlines() if "==" in line}
