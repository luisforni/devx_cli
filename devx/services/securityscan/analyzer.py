...
def analyze_meta_http_equiv(html: str) -> Dict[str, str]:
    ...
    try:
        ...
        for meta in soup.find_all("meta"):
            hev = (meta.get("http-equiv") or meta.get("http_equiv") or "").strip().lower()
...
def analyze_headers(url: str, headers: httpx.Headers) -> List[Tuple[str, str, str]]:
    ...
    if xcto is not None and xcto.lower() != "nosniff":
        issues.append(("weak", "X-Content-Type-Options", f"Valor '{xcto or 'N/A'}' (recomendado: nosniff)"))

    xfo = headers.get("X-Frame-Options")
    if xfo is not None and xfo.lower() not in ("deny", "sameorigin"):
        issues.append(("weak", "X-Frame-Options", f"Valor '{xfo}' (recomendado: DENY o SAMEORIGIN)"))
    ...
    if csp:
        ...
        if "*" in csp and "script-src" in csp:
            issues.append(("weak", "Content-Security-Policy", "script-src demasiado permisivo"))
...
def analyze_cookies(headers: httpx.Headers) -> List[Tuple[str, str, str]]:
    ...
    for ck in raw_list:
        ...
        if "secure" not in flags:
            issues.append(("weak", f"Set-Cookie {name}", "Falta flag Secure (solo sobre HTTPS)"))
        if "httponly" not in flags:
            issues.append(("weak", f"Set-Cookie {name}", "Falta flag HttpOnly"))
        if "samesite" not in flags:
            issues.append(("weak", f"Set-Cookie {name}", "Falta flag SameSite (Lax/Strict)"))
    return issues
...
def analyze_tls(info: Dict[str, Any]) -> List[Tuple[str, str, str]]:
    ...
    exp: datetime.datetime | None = info.get("expires_at")
...
def scan_ports(host: str, ports: List[int] = COMMON_WEB_PORTS, timeout: float = 0.3) -> List[int]:
    open_ports: List[int] = []
    for p in ports:
        s = socket.socket()
        try:
