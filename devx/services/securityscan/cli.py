...
def _compose_output_path(reports_dir: Path, host: str, json_out_opt: str | None) -> Path:
    if json_out_opt:
        p = Path(json_out_opt)
        if p.is_absolute():
            return p
...
def run(
    ...
    json_out: str = typer.Option(None, "--json", help="Output JSON file name or absolute path"),
    out_dir: str = typer.Option(None, "--out-dir", help="Directory to store reports (overrides DEVX_REPORTS_DIR)"),
    fetch_html: bool = typer.Option(False, "--fetch-html", help="Fetch HTML and detect <meta http-equiv> CSP/Referrer-Policy"),
):
    logger = log.setup()
    ...
    http_proto_note = "(HTTP/1.1)" if (force_http1 or not H2_AVAILABLE) else "(HTTP/2→1.1 fallback)"
    print(Panel.fit(f"[bold]🛡️ Security Scan[/bold] {http_proto_note}\n{url}", border_style="cyan"))
    ...
    try:
        ...
        if fetch_html and html:
            ...
            for k, v in meta_overrides.items():
                header_issues = [i for i in header_issues if not (i[0] == "missing" and i[1] == k)]
        ...
        if header_issues:
            ...
            for t, h, d in header_issues:
                ...
                findings.append({"type": t, "area": "header", "name": h, "detail": d, "severity": _severity_for(t, h)})
            print(table_h)
        else:
        ...
        if cookie_issues:
            ...
            for t, c, d in cookie_issues:
                ...
                findings.append({"type": t, "area": "cookie", "name": c, "detail": d, "severity": _severity_for(t, c)})
            print(table_c)
        ...
        if techs or leaks:
            ...
            if techs:
                ...
                findings.append({"type": "info", "area": "fingerprint", "name": "detected", "detail": ", ".join(techs), "severity": "info"})
            for k, v in leaks:
                table_f.add_row(f"Leak: {k}", v)
                findings.append({"type": "info", "area": "leak", "name": k, "detail": v, "severity": "low"})
            print(table_f)
        ...
        if not quick:
            ...
            if open_ports:
                ...
                print(table_p)
                findings.append({"type": "info", "area": "ports", "name": "open_ports", "detail": open_ports, "severity": "info"})
            ...
            if url.startswith("https://"):
                try:
                    ...
                    table_t.add_row("Expires", exp.strftime("%Y-%m-%d %H:%M:%S UTC") if exp else "N/A")
                    print(table_t)
                    for t, a, d in tls_issues:
                        findings.append({"type": t, "area": a, "name": a, "detail": d, "severity": _severity_for(t, a)})
                except Exception as e:
        ...
        total_findings = sum(1 for f in findings if f["severity"] in ("low", "medium", "warn", "critical"))
        if tls_issues:
        ...
        if total_findings == 0:
            ...
            print(f"[bold][yellow]⚠ Scan finalizado con {total_findings} hallazgos[/yellow][/bold]")
            raise typer.Exit(code=1)
