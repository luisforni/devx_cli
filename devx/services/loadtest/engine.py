import asyncio, time, json, statistics as stats
import httpx
...
async def run_load(url, rps, duration, method, timeout, headers, body, verify_ssl):
    async with httpx.AsyncClient(timeout=timeout, verify=verify_ssl, follow_redirects=True) as client:
        ...
        for sec in range(duration):
            ...
            tasks = [client.request(method, url, headers=headers, json=body if isinstance(body, dict) else None,
                                    content=None if isinstance(body, dict) else body) for _ in range(rps)]
