import asyncio, json, statistics as stats
import typer
...
def run(
    ...
    lat, codes, errors = asyncio.run(run_load(url, rps, duration, method, timeout, hdrs, body, verify_ssl))

    total = rps * duration
    ok = sum(1 for c in codes if 200 <= c < 400)
    table = Table(title="Results"); table.add_column("Metric"); table.add_column("Value", justify="right")
    table.add_row("Total", str(total)); table.add_row("Success (2xx/3xx)", str(ok))
